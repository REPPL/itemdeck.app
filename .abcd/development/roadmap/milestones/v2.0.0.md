...
1. **Foundation** - Robust i18n infrastructure with type-safe translations
2. **German Launch** - First non-English language fully supported
3. **RTL Ready** - Full Arabic and Hebrew support with mirrored UI
4. **Icon System** - JSON-based registry replacing inline SVGs
5. **Performance** - Lazy-loaded translations within bundle budget
...
Work proceeds sequentially due to dependencies:

| Track | Focus | Features | Est. Hours |
|-------|-------|----------|------------|
| **A** | i18n Foundation | F-075 | 30-40 |
| **B** | Icon Registry | F-076 | 15-20 |
| **C** | RTL Support | F-077 | 35-45 |
...
Track A (Foundation) ────────────────────────────────────┐
    ...
    └── F-075: Internationalisation Foundation            │
           • react-i18next setup                         │
           • TypeScript translation keys                 │
           • English + German translations               ├──► Sequential
           │                                             │
           ▼                                             │
Track B (Icons)                                          │
    │                                                     │
    └── F-076: Icon Configuration Registry               │
           • JSON-based icon definitions                 │
...
- Use react-i18next with namespace-based translations
- TypeScript type safety via generated types from translation files
- Lazy load translation bundles per language
- Start with en-GB, en-US, de-DE

### F-076: Icon Configuration Registry

- JSON registry replaces inline SVGs across codebase
- Universal `<Icon name="..." />` component
- Add `rtlMirror: true` flag for directional icons
- Must complete before RTL work begins

### F-077: RTL Language Support

- Migrate all CSS to logical properties (`margin-inline-start` not `margin-left`)
- Approximately 100+ CSS module files to update
- Arabic and Hebrew require native speaker validation
- Visual regression tests for RTL mode
