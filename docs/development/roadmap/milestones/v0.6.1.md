...
|----|---------|------------|--------|
| F-042 | Collection Display Driver | Large | 📋 Planned |
| F-043 | Settings Panel Sub-tabs | Medium | 📋 Planned |
| F-044 | Random Card Sampling | Small | 📋 Planned |
| - | Settings Audit & Verification | Medium | 📋 Planned |
...
- [ ] Cards render title/subtitle from `display.card.front.title`/`subtitle` paths
- [ ] Badges render from `display.card.front.badge`/`secondaryBadge` paths
- [ ] Card back renders from `display.card.back` configuration
- [ ] Field path expressions work with dot notation and array filters
- [ ] Missing fields show placeholder or hide gracefully
- [ ] Existing v1 collection.json format works without changes

### F-043 (Settings Panel Sub-tabs)
- [ ] "Cards" tab splits into General/Front/Back sub-tabs
- [ ] All existing settings retain functionality
- [ ] Tab state persists during session
- [ ] Keyboard navigation works within sub-tabs

### F-044 (Random Card Sampling)
- [ ] "Max visible cards" setting in General sub-tab
- [ ] Dropdown shows "All" plus 1 to N (collection size)
- [ ] Random selection applied on shuffle
- [ ] "All" restores full deck
- [ ] Setting persists in localStorage
- [ ] Value clamps when switching to smaller collections

### Settings Audit
- [ ] Every setting in the panel has a visible effect on the UI
- [ ] All display-related settings accessible via Settings panel
- [ ] All settings persist correctly in localStorage
- [ ] Settings load correctly on page refresh
- [ ] No orphaned settings (settings that exist but do nothing)
- [ ] No hidden settings (functionality that should be configurable but isn't)

### R-003 (External Scores Research)
- [ ] Research document created in `docs/development/research/`
- [ ] Legal requirements documented for each source
- [ ] API availability and rate limits documented
- [ ] Recommendation provided for itemdeck integration
