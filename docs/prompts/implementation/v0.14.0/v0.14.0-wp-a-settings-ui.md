...
Implement a draft state pattern for settings so changes only apply when the user clicks "Accept", and polish UI consistency across all dialogs and modals.
...
Additionally, button styling across dialogs/modals is inconsistent, with some buttons missing borders or using inconsistent hierarchy.
...
5. Audit and standardise button styles across all modals/dialogs
...
### 3. Cache Consent Dialog UX (F-092)
...
**Scope:** All modals and dialogs across the application.
...
- [ ] Cache consent dialog has improved copy and button layout
- [ ] "(can change in Settings)" hint appears on Never cache option
- [ ] Active mechanic not duplicated in selection list
- [ ] All modals/dialogs use consistent button hierarchy
