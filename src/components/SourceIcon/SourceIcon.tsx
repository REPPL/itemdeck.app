...
/**
 * Extract the lowercased hostname from a URL, or null if it does not parse as
 * an absolute URL. Matching on the hostname (rather than a substring of the
 * whole URL) stops an untrusted collection from branding an attacker link with
 * a trusted source's icon and name via a path like
 ...
 * substring/`endsWith` check would still be fooled by `wikipedia.org.evil.com`,
 * so the match is anchored on the dot-delimited boundary.
 */
...
/**
 * Source domains and their corresponding icons.
 */
...
const ADDITIONAL_SOURCES: {
  ...
  {
    domain: "myabandonware.com",
    title: "My Abandonware",
    shortName: "A",
  },
  {
    domain: "video-games-museum.com",
...
export function SourceIcon({ url, source, className }: SourceIconProps) {
  const match = useMemo(() => {
    // First check the URL's host for known sources
    const hostname = getHostname(url);
    if (hostname) {
      for (const pattern of SOURCE_PATTERNS) {
        if (hostMatchesDomain(hostname, pattern.domain)) {
          return pattern;
        }
      }
    }
...
export function isKnownSource(url: string): boolean {
  ...
  return SOURCE_PATTERNS.some((pattern) =>
    hostMatchesDomain(hostname, pattern.domain)
  );
}
...
export function getSourceName(url: string): string | undefined {
  ...
  const match = SOURCE_PATTERNS.find((pattern) =>
    hostMatchesDomain(hostname, pattern.domain)
  );
  if (match) return match.title;

  const additionalMatch = ADDITIONAL_SOURCES.find((s) =>
    hostMatchesDomain(hostname, s.domain)
  );
...
export function getSourceShortName(
  ...
  for (const source of ADDITIONAL_SOURCES) {
    if (hostMatchesDomain(hostname, source.domain)) {
